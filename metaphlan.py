...
class Bowtie2Controller(MappingController):
    ...
    def get_bowtie2cmd(self):
        ...
        if ',' not in self.inp:
            ...
            R1_fna, R2_fna = self.inp.split(',')
            samplename = os.path.basename(R1_fna)
            samplename = samplename.split('.')[0].split('_R1')[0]
            tmp_R1_fastq = f"{samplename}.tmp.R1.fastq"
            tmp_R2_fastq = f"{samplename}.tmp.R2.fastq"
            bowtie2_cmd += ["-1", tmp_R1_fastq]
            bowtie2_cmd += ["-2", tmp_R2_fastq]
        ...
        try:
            if self.inp:
                if ',' in self.inp:
                    ...
                    tmp_R1_fastq = f"{samplename}.tmp.R1.fastq"
                    tmp_R2_fastq = f"{samplename}.tmp.R2.fastq"
                    R1_stderr = f"{samplename}.sterr.R1"
                    
                    # run read_fastx.py on R1 input, write a R1 tmp file and also a R1 stderr file
                    with open(tmp_R1_fastq, "w") as out_f, open(R1_stderr, "w") as err_f:
                        subp.run([read_fastx, '-l', str(self.read_min_len), R1_fna], stdout=out_f, stderr=err_f, check=True)
                        
                    # doesn't write a stderr file because the informations there (nreads and avg_read_length)
                    # are the same as the R1 file
                    with open(tmp_R2_fastq, "w") as out_f:
                        subp.run([read_fastx, '-l', str(self.read_min_len), R2_fna], stdout=out_f, check=True)
            ...
            if ',' in self.inp:
                p = subp.Popen(self.get_bowtie2cmd(), stdout=subp.PIPE)    
            else:
            ...
            for line in p.stdout:
                ...
                # Normalize QNAME (read name) by stripping everything after '__'
                qname = o[0].split('__')[0]
                
                if self.check_hq_mapping(o):
                    if qname in self.seen_mates:
                        prev_o = self.seen_mates[qname]
                        
                        if o[2] != prev_o[2]:
                            #print("Il prev hitta una sequenza diversa")
                            # Mates map to different references → write both
                            outf.write("\t".join([prev_o[0], prev_o[2].split('/')[0]]) + "\n")
                            outf.write("\t".join([o[0], o[2].split('/')[0]]) + "\n")
                        else:
                            # Mates map to same marker → write only one
                            outf.write("\t".join([o[0], o[2].split('/')[0]]) + "\n")
                
                        del self.seen_mates[qname]
                    
                    else:
                        # Store this read until we see its mate
                        self.seen_mates[qname] = o
            ...
            if ',' in self.inp:
                with open(R1_stderr, "r") as err_f:
                    line = err_f.readline().strip()  # Read the first line and remove trailing newline
                    nreads, avg_read_len, _ = line.split("\t")
            else:
            ...
            if tmp_R1_fastq:
                subp.run(["rm", "-f", tmp_R1_fastq])
            if tmp_R2_fastq:
                subp.run(["rm", "-f", tmp_R2_fastq])
            if R1_stderr:
                subp.run(["rm", "-f", R1_stderr])
